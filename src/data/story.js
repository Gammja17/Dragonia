...
export const LESSONS = [
    ...
    { id: 'L4', level: 5,  title: '넷째 수련: 바람 읽기', skill: 'WING_GUST',    drill: { type: 'DODGE', time: 28, rate: 0.4 },
      intro: "이번에는 불씨를 더 촘촘하게 던질 거다. 그래도 불씨 사이에는 반드시 빠져나갈 길이 있으니까 그걸 찾아봐라." },
    { id: 'L5', level: 7,  title: '다섯째 수련: 숨 고르기',   skill: 'HEAL',         drill: { type: 'TARGETS', count: 8, hp: 130, time: 60 },
      intro: "싸움이 길어지면 숨 고를 줄 모르는 놈부터 쓰러진다. 오늘은 허수아비가 여덟이니까 서둘러야 할 거다." },
    { id: 'L6', level: 9,  title: '여섯째 수련: 용의 숨',     skill: 'FLAME_BREATH', drill: { type: 'DUEL', hp: 260 },
      intro: "오늘은 나랑 직접 붙는다. 봐줄 생각 없으니까 네가 가진 걸 전부 써 봐라." },
    { id: 'L7', level: 11, title: '일곱째 수련: 강철 비늘',   skill: 'IRON_SCALE',   drill: { type: 'DODGE', time: 36, rate: 0.3 },
      intro: "살다 보면 못 피하는 공격도 있어서 그럴 땐 버텨야 한다. 버티는 건 끝나고 알려 줄 테니까 오늘은 일단 피하는 것부터 해라." },
...
export const TRIALS = [
    ...
    //   why       아직 안 될 때 스승이 하는 말
    {
        ...
        intro: "날개가 근질거리는 모양이구나. 좋다, [어린 용] 소리를 들을 만한지 어디 한번 보자.",
    },
    {
        stage: 2, hp: 300,
...
// 어둠의 길(히든). 마을 어귀에서 스승을 이겼을 때와 졌을 때 (systems/story.js 의 darkDuel)
export const DARK_ROUTE = {
    win: [
        { who: 'Kairon', text: "…허, 내가 졌구나. 가르치기는 제대로 가르친 모양이다." },
        { who: 'Kairon', text: "네가 그렇게 정했다면 가라. 다만 문이라는 건 밖에서 부수는 것보다 안에서 닫아 버리는 게 더 아프다는 것만은 기억해 둬라." },
        { who: '나', text: "(마을 용들이 하나둘 고개를 숙였는데, 나라만은 끝까지 고개를 들고 나를 쳐다보고 있었다.)" },
    ],
    // 졌다. 스승이 끌고 돌아온다 — 교화. 본 이야기(m6)로 되돌아간다
    lose: [
        { who: 'Kairon', text: "이제 그만 일어나라. 집에 가자." },
        { who: 'Kairon', text: "형이 너한테 한 말은 예순 해 전에 나도 다 들은 말이다. 그때 나는 아무 대답도 못 하고 형을 혼자 보냈는데, 너까지 그렇게 보낼 수는 없더라." },
        { who: 'Nara', text: "야, 이 바보야! 너 진짜 저쪽으로 가려고 했어? …다음에 또 그러면 그때는 스승님 말고 내가 팬다." },
        { who: 'Poco', text: "(포코가 아무 말 없이 다가와서 내 꼬리를 꼭 붙잡았다.)" },
        { who: 'Elder', text: "돌아와 주었으니 그걸로 되었단다. 이 일은 아무도 다시 꺼내지 않을 게다. 내가 그리 일러 두마." },
        { who: '나', text: "(등 뒤의 하늘을 덮고 있던 잿빛 날개들이 어느새 남동쪽으로 물러가고 없었다.)" },
    ],
};
...
export const SCENES = [
    ...
    // (예전 ch2 "자라는 날개"는 퀘스트 m2 와 같은 말을 두 번 해서 뺐다)
    {
        ...
        lines: [
            ...
            { who: 'Tiamat', text: "미라가 다 돌아봤는데 다친 용은 없대. 너는 어디 긁힌 데 없어?" },
    ...
    },
    {
        ...
        lines: [
            ...
            { who: 'Kairon', text: "일어났냐. …어제 일은 고마웠다는 말을 하려고 왔다." },
    ...
    {
        id: 'dream_ice', title: '재 냄새가 나는 꿈', when: s => s.bossesDefeated.MORGATH && s.story.scenes.includes('ch4'),
        lines: [
            { who: '???', text: "(꿈속에서 누군가 내 옆에 내려앉았다. 얼굴은 보이지 않는데 재 냄새가 난다.)" },
            { who: '???', text: "「숨결이 하나 늘었구나. 너는 그걸 받았다고 들었다. 나는 같은 것을 얻으려고 스승을 넘어뜨려야 했는데 말이다.」" },
            { who: '???', text: "「부럽다는 말을 하려는 게 아니라, 받은 것은 언젠가 값을 치르게 된다는 걸 알려 주려는 거다.」" },
            { who: '나', text: "(눈을 떴을 때는 아무도 없었는데, 굴 안에 재 냄새가 희미하게 남아 있었다.)" },
        ],
    },
    {
        id: 'dream_thunder', title: '두 번째 꿈', when: s => s.bossesDefeated.ZALGORA && s.story.scenes.includes('ch5'),
        lines: [
            { who: '???', text: "「그 형제가 너한테는 제 숨결을 건네주었다지. 나한테는 빼앗겼다고 하더냐.」" },
            { who: '???', text: "「틀린 말은 아니다. 다만 그 둘이 나를 따라나선 건, 굶주린 아이들한테 너희 마을이 아무것도 해 주지 못했기 때문이라는 것도 알아 둬라.」" },
            { who: '???', text: "「네 마을 어른들은 너한테 늘 절반만 말해 준다. 나머지 절반이 궁금해지거든 나를 찾아와라.」" },
            { who: '나', text: "(이번에도 얼굴은 보지 못했다. 목소리는 이상하게 스승님과 닮아 있었다.)" },
        ],
    },
    {
        id: 'dream_city', title: '세 번째 꿈', when: s => s.quests.done.includes('m5c') && s.story.scenes.includes('ch6'),
        lines: [
            { who: '???', text: "「내가 태운 도시를 보고 왔구나. 끔찍했을 거다. 나도 그날 이후로 그 냄새를 잊어 본 적이 없으니까.」" },
            { who: '???', text: "「그런데 그날 내가 돌아서 버렸다면, 지금 너를 기른 그 마을은 남아 있지 않았을 거다. 너라면 그 자리에서 어떻게 했겠느냐.」" },
            { who: '나', text: "(대답을 하려는 순간에 잠이 깼고, 내가 뭐라고 대답하려 했는지는 기억나지 않는다.)" },
        ],
    },
    {
        ...
        lines: [
            ...
            { who: 'Kairon', text: "가서 무슨 말을 해야 할지 몰라서 나는 예순 해 동안 못 갔다. 그걸 너한테 떠넘기는 거라 미안하다." },
            { who: 'Nara', text: "(수련장 밖에서 나라가 기다리고 있었다.) 갈 거지? 말 안 해도 알아." },
            { who: 'Nara', text: "돌아와. 나 아직 너한테 한 번도 못 이겼단 말이야. 이기고 도망가는 게 어딨어." },
        ],
    },
    {
        // 수호룡: 이그나르를 끝냈다 (옛 세이브는 route 가 없으므로 이쪽으로 온다)
        id: 'ending', title: '마지막 장. 새 수호룡', when: s => s.quests.done.includes('m6') && s.story.route !== 'redeem' && s.story.route !== 'dark', place: 'VILLAGE',
        lines: [
            { who: '???', text: "(꿈을 꿨는데 이번에는 떨어지지 않았다. 구름 위를 아주 오랫동안 날았다.)" },
            { who: 'Kairon', text: "형을 멈춰 줘서 고맙다. 마지막에 하늘이 넓더라고 했다지. 그 말을 들으려고 내가 예순 해를 살았나 보다." },
            { who: 'Elder', text: "어디서 떨어졌든 너는 여기서 자란 우리 마을 아이란다. 오늘부터는 네가 이 마을의 수호룡이니, 나는 이제 좀 쉬어도 되겠구나…" },
            { who: 'Nara', text: "수호룡이 됐다고 봐줄 줄 알았으면 오산이야. 나 내일부터 너 이길 때까지 매일 덤빌 거니까 각오해." },
            { who: 'Poco', text: "수호룡님! …그래도 나랑 술래잡기는 계속해 줄 거지? 응?" },
            { who: '나', text: "(이그나르도 나처럼 하늘에서 떨어진 용이었다. 다른 게 있다면, 나한테는 돌아올 마을이 있었다는 것뿐이다.)" },
            { who: '나', text: "(이야기는 여기서 끝나지만, 마을의 하루는 내일도 이어진다.)" },
        ],
    },
    {
        // 교화: 이그나르를 업고 돌아왔다
        id: 'ending_redeem', title: '마지막 장. 돌아온 형', when: s => s.quests.done.includes('m6') && s.story.route === 'redeem', place: 'VILLAGE',
        lines: [
            { who: '나', text: "(아침에 나와 보니 마을 어귀에 용들이 모여 있다. 다들 멀찍이 서서, 바위에 기대앉은 검은 용을 보고 있다.)" },
            { who: 'Kairon', text: "…형." },
            { who: 'IGNAR', text: "많이 늙었구나, 카이론. 잔소리가 늘었다는 소문은 화산까지 들리더라." },
            { who: 'Kairon', text: "형이 떠나던 날 물었던 거 말이오. 뒤에서 보는 하늘은 좁지 않으냐고. 넓더이다. 뒤에서 보니까 형도 보이고, 이 녀석도 보이고." },
            { who: 'IGNAR', text: "…그 대답을 들으려고 예순 해가 걸렸구나." },
            { who: 'Elder', text: "이그나르야. 네가 한 일을 없던 일로 해 줄 수는 없단다. 죽은 이들이 있으니까. 허나 너를 그리 만든 게 우리였다는 것도 없던 일로 하지는 않으마." },
            { who: 'Elder', text: "마을 밖 빈 굴을 하나 내어 주마. 거기서부터 다시 시작해 보자꾸나. 삼백 년 전에 그랬던 것처럼 말이다." },
            { who: 'Poco', text: "(한참을 티아맷 뒤에 숨어서 보고 있던 포코가 고기 한 점을 들고 조심조심 다가간다.) …저기, 이거 먹을래요?" },
            { who: 'IGNAR', text: "……삼백 년 전에도 누가 이렇게 고기를 내밀었었지. 고맙다, 꼬마야." },
            { who: '나', text: "(이야기는 여기서 끝나지만, 마을의 하루는 내일도 이어진다. 내일부터는 식구가 하나 더 늘어서.)" },
        ],
    },
];
