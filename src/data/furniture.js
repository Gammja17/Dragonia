...
//
//   tile   시트의 [칸x, 칸y] (16px 단위)
//   sheet  어느 시트에서 떠 오는가. 없으면 'dungeon'(Kenney 16px 아이콘),
//          'inner' 는 zelda-like 팩의 실내 가구 시트 — 훨씬 크고 자세하다
...
export const FURNITURE = {
    ...
    SHELF:   { name: '책 선반',        tile: [3, 12], sheet: 'inner', span: [1, 2], cozy: 3, cost: { gold: 90, ORE: 1 },  note: '엘더가 빌려준 두루마리들.' },
    ...
    PICTURE: { name: '액자',           tile: [16, 0], sheet: 'inner', span: [2, 1], cozy: 3, wall: true, cost: { gold: 120, HIDE: 2 }, note: '마을이 내려다보이는 그림.' },
    PILLAR:  { name: '돌기둥',         tile: [6, 1], span: [1, 2], cozy: 2, cost: { gold: 110, ORE: 6 },          note: '천장을 받쳐 준다. 사실은 멋으로.' },
    POTIONS: { name: '약병 선반',      tile: [4, 12], sheet: 'inner', span: [1, 2], cozy: 1, cost: { gold: 35 },           note: '무슨 맛인지는 모르겠다.' },
    ORB:     { name: '빛나는 구슬',    tile: [6, 8], span: [1, 1], cozy: 4, light: '#9fe3ff', cost: { gold: 150, ORE: 3, FANG: 3 }, note: '밤에도 굴이 환하다.' },
    BRAZIER: { name: '화로',           tile: [10, 12], sheet: 'inner', span: [2, 1], cozy: 4, light: '#ffb347', cost: { gold: 130, ORE: 5 }, note: '불을 피워 두면 겨울에도 따뜻하다.' },

    // ---- 실내 가구 시트(zelda-like, CC0)에서 새로 들인 것들 ----
    // 값은 기존 것들의 눈금에 맞춰 대충 잡아 둔 것이다. 필요하면 주무르면 된다.
    BED:     { name: '푹신한 잠자리',  tile: [16, 1], sheet: 'inner', span: [1, 3], cozy: 5, cost: { gold: 140, HIDE: 4 },     note: '마른 풀보다야 낫다.' },
    CHAIR:   { name: '등받이 의자',    tile: [15, 4], sheet: 'inner', span: [1, 3], cozy: 2, cost: { gold: 55 },               note: '기대앉으면 꼬리가 편하다.' },
    LONGTABLE: { name: '긴 탁자',      tile: [14, 7], sheet: 'inner', span: [3, 2], cozy: 3, cost: { gold: 120 },             note: '여럿이 둘러앉을 수 있다.' },
    CUPBOARD: { name: '찬장',          tile: [6, 10], sheet: 'inner', span: [3, 2], cozy: 3, cost: { gold: 150, ORE: 2 },      note: '넣어 둘 데가 넉넉하다.' },
    DRESSER: { name: '서랍장',         tile: [10, 7], sheet: 'inner', span: [3, 2], cozy: 3, cost: { gold: 135 },             note: '뭘 넣었는지는 잊었다.' },
    FIREPLACE: { name: '벽난로',       tile: [12, 11], sheet: 'inner', span: [2, 3], cozy: 6, light: '#ffb347', cost: { gold: 220, ORE: 8 }, note: '굴 전체가 따뜻해진다.' },
    PLANT:   { name: '화분 나무',      tile: [8, 12], sheet: 'inner', span: [1, 2], cozy: 2, cost: { gold: 50 },              note: '굴 안에서도 잘 자란다.' },
    WINDOW:  { name: '창',             tile: [9, 4], sheet: 'inner', span: [2, 2], cozy: 4, wall: true, cost: { gold: 180, ORE: 3 }, note: '바위를 뚫어 바깥을 냈다.' },
    NIGHTSKY: { name: '밤하늘 그림',   tile: [14, 0], sheet: 'inner', span: [2, 1], cozy: 3, wall: true, cost: { gold: 110, HIDE: 2 }, note: '떨어지던 날의 하늘.' },
};

/** 그 가구를 어느 시트에서 떠 오는가 */
export const furnitureSheet = (f) => (f && f.sheet) || 'dungeon';
